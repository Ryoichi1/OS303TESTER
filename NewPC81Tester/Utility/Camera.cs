                ...
                using (var cap = Cv.CreateCameraCapture(CameraNumber)) // カメラのキャプチャ
                {
                    ...
                    while (StopFlag)             // 任意のキーが入力されるまでカメラ映像を表示
                    {
                        try
                        {
                            ...
                            {
                                var imbuff = im.Clone();
                                var Gbuff = Gray(imbuff);
                                dis.BeginInvoke(new Action(() =>
                                {
                                    WriteableBitmapConverter.ToWriteableBitmap(Gbuff, grayBuff);// カメラからフレーム(画像)を取得
                                    source = grayBuff;
                                }));
